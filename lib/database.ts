...
export class DatabaseService {
  ...
  static async getUsersBySchool(schoolId: string) {
    await connectToDatabase()
    return await User.find({ school_id: schoolId, is_active: true }).populate('school')
  }

  static async getUsersByRole(schoolId: string, role: string) {
    await connectToDatabase()
    return await User.find({ school_id: schoolId, role, is_active: true }).populate('school')
  }
